...
export default function CobranzaDetail() {
  const { id } = useParams()
  const navigate = useNavigate()

  const { data: cobranza, isLoading } = useQuery({
    queryKey: ['cobranza-detail', id],
    queryFn: async () => {
      const { data: res } = await api.get(`/admin/cobranzas/${id}`)
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <span className={cn('inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium',
            cobranza.erpSynced ? 'bg-success/10 text-success' : 'bg-warning/10 text-warning')}>
            {cobranza.erpSynced ? 'Sincronizado' : 'Pendiente ERP'}
          </span>
            ...
            <tbody>
              {cobranza.medios.map((medio) => (
                <tr key={medio.id} className="border-b border-border last:border-0">
                  ...
                  <td className="py-2 text-xs text-muted-foreground">{medio.fechaVencimiento ? formatDateTime(medio.fechaVencimiento) : '—'}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
